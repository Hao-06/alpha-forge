...
```
[实时行情：BTC / ETH / SOL]   +   [OKX funding rate 全市场扫描]
              ↓                              ↓
┌────────────────────────────────────────────────────────┐
│ 📊 行情判断 Agent      ← DeepSeek-R1-0528             │
│    输出：市场 regime + 概率分布                          │
├────────────────────────────────────────────────────────┤
│ 🎯 策略选择 Agent      ← GPT-4o                        │
│    输出：从 5 策略库中选/混合 + 权重 + 微调建议         │
├────────────────────────────────────────────────────────┤
│ 🛡️ 风险审核 Agent     ← GPT-4o-mini                   │
│    输出：风险评分 + 否决/通过                            │
├────────────────────────────────────────────────────────┤
│ 💱 资金费率套利 Agent  ← GPT-4o                        │
│    输出：Delta-Neutral 套利推荐                         │
└────────────────────────────────────────────────────────┘
              ↓
🧠 决策官（综合）→ 交易信号 JSON + R1 思维链 + OKX dry-run 订单
```
...
- **GMI Cloud Inference Engine API** —— 多模型协作核心
  - DeepSeek-R1-0528（推理）/ GPT-4o（综合）/ GPT-4o-mini（风控）
